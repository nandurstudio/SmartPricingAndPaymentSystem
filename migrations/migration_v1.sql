...
INSERT INTO m_role (txtRoleName, txtRoleDesc, txtRoleNote, bitStatus, txtGUID) VALUES
('Administrator', 'Full access to the system', 'Initial system administrator role', 1, 'role_admin'),
('User', 'Standard user role', 'Default user permissions', 1, 'role_user');

-- Seeder data for m_service_types
INSERT INTO m_service_types (guid, name, slug, description, icon, category, is_system, is_approved, default_attributes, is_active, created_date, created_by)
VALUES
('guid_futsal', 'Futsal', 'futsal', 'Futsal court booking and management service', 'fas fa-futbol', 'sports', 1, 1, '{"court_size":{"type":"select","label":"Court Size","options":["small","medium","large"],"required":true},"duration":{"type":"number","label":"Duration (hours)","min":1,"max":4,"required":true}}', 1, NOW(), 'system'),
('guid_villa', 'Villa Rental', 'villa-rental', 'Villa and accommodation booking service', 'fas fa-home', 'accommodation', 1, 1, '{"room_count":{"type":"number","label":"Number of Rooms","min":1,"max":20,"required":true},"guest_count":{"type":"number","label":"Number of Guests","min":1,"max":50,"required":true}}', 1, NOW(), 'system');

-- Seeder data for m_tenants
INSERT INTO m_tenants (guid, name, slug, service_type_id, owner_id, subscription_plan, status, settings, is_active, created_date, created_by)
VALUES
('guid_alpha', 'Alpha Futsal Center', 'alpha-futsal', 1, 1, 'basic', 'active', '{"currency":"IDR"}', 1, NOW(), 'system'),
('guid_omega', 'Omega Villa Resort', 'omega-villa', 2, 2, 'premium', 'active', '{"check_out_time":"12:00"}', 1, NOW(), 'system');

-- Seeder data for m_user
INSERT INTO m_user (intRoleID, txtUserName, txtFullName, txtEmail, txtPassword, bitActive, txtCreatedBy, dtmCreatedDate, txtUpdatedBy, dtmUpdatedDate, txtGUID, txtPhoto, dtmJoinDate)
VALUES
(3, 'futsal.owner', 'Futsal Owner', 'futsal@example.com', '$2y$10$examplehash', 1, 'system', NOW(), 'system', NOW(), 'user_futsal', 'default.png', NOW()),
